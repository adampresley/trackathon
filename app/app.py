...
if config.DEBUG:
	...
	if config.BIND_TO_OUTSIDE_IP:
		run(app = app, host = outsideIP[0], port = config.BIND_TO_PORT, server="gunicorn", workers = 4, proc_name = "trackathon", pidfile = "/tmp/trackathon-gunicorn", daemon = True)
	else:
		run(app = app, host = "localhost", port = config.BIND_TO_PORT, server="gunicorn", workers = 4, proc_name = "trackathon", pidfile = "/tmp/trackathon-gunicorn", daemon = True)
